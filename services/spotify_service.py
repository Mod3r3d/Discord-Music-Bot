...
---------------------------
Sử dụng Token từ Embed Iframe để chống Block IP 100%.
Cho phép tải hàng ngàn bài hát một cách dễ dàng.
"""
import json
import re
import time
...
class SpotifyService:
    ...
    async def _get_anonymous_token(self) -> Optional[str]:
        ...
        # TRÍCH XUẤT TOKEN TỪ IFRAME BÊN THỨ 3 (Không bao giờ bị block)
        try:
            async with aiohttp.ClientSession(headers=HEADERS) as session:
                async with session.get("https://open.spotify.com/embed/track/4cOdK2wGLETKBW3PvgPWqT", timeout=15) as resp:
                    if resp.status == 200:
                        ...
                        if match:
                            ...
                            if token:
                                ...
                                log.info("Đã lấy thành công Token từ Iframe!")
                                return self._anon_token
        except Exception as exc:  # noqa: BLE001
            log.warning("Lỗi trích xuất token Spotify Embed: %s", exc)
        return None
    ...
    async def _fetch_embed_fallback(self, type_: str, item_id: str, requester_id: int) -> Tuple[str, List[Track]]:
        ...
        async with aiohttp.ClientSession(headers=HEADERS) as session:
            async with session.get(embed_url, timeout=10) as resp:
                if resp.status != 200: raise RuntimeError("Spotify từ chối kết nối (HTTP 403).")
                html = await resp.text()

        match = re.search(r'<script id="__NEXT_DATA__" type="application/json">(.*?)</script>', html)
        if not match: raise RuntimeError("Không thể giải mã dữ liệu Spotify Embed.")
        ...
        for item in track_list:
            ...
            tracks.append(Track(
                ...
                webpage_url=f"https://open.spotify.com/track/{item.get('uri', '').split(':')[-1]}" if item.get("uri") else f"https://open.spotify.com/{type_}/{item_id}",
    ...
    async def fetch_track(self, track_id: str, requester_id: int) -> Optional[Track]:
        ...
        async with aiohttp.ClientSession(headers={**HEADERS, "Authorization": f"Bearer {token}"}) as session:
            async with session.get(f"https://api.spotify.com/v1/tracks/{track_id}", timeout=10) as resp:
                if resp.status != 200:
                    _, tracks = await self._fetch_embed_fallback("track", track_id, requester_id)
                    return tracks[0] if tracks else None
                t = await resp.json()
    ...
    async def fetch_collection(self, type_: str, item_id: str, requester_id: int) -> Tuple[str, List[Track]]:
        token = await self._get_anonymous_token()
        if not token: return await self._fetch_embed_fallback(type_, item_id, requester_id)
